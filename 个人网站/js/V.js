...
.addView('tab', function (model, template) {
    ...
    for (var i = 0; i < d.data.data.length; i++) {
        if (i == 1 || i == 3) {
            ...
            for(var j=0;j<d.data.data[i].subhead.length;j++){
                liTpl1 += '<li><a>' + d.data.data[i].subhead[j] + '</a></li>';
            }
            liTpl1 += '</ul></li>';
    ...
    dom.html(html);
    dom.appendTo('.header');
    $(".header").find(".menu_list").eq(0).find("a").attr("href","index.html");
    $(".header").find(".menu_list").eq(1).find("a").attr("href", "guanyuwo.html");
    $(".header").find(".menu_list").eq(1).find("ul").find("li").eq(0).attr("href", "guanyuwo.html");
    $(".header").find(".menu_list").eq(1).find("ul").find("li").eq(1).attr("href", "youqinghulian.html");
    $(".header").find(".menu_list").eq(4).find("a").attr("href", "yule.html");
    $(".header").find(".menu_list").eq(6).find("a").attr("href", "liuyan.html");
    return dom;
})
